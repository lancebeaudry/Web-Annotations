...
import { resolveProject, listFeedback, listPages, getFeedback, actOn, STATUSES, LABELS, EFFORTS, type Project } from "../_shared/feedback.ts";
...
const TOOLS = [
  {
    ...
    inputSchema: { type: "object", properties: {
      ...
      label: { type: "string", enum: [...LABELS], description: "Only items carrying this label." },
    } },
  },
  ...
  {
    name: "set_status",
    description: "Set an item's status. Use 'resolved' only after the requested change is actually made; 'in_progress' when you start; 'waiting' when the client has to decide or supply something (say what you need in the reply); 'wont_fix' with a reply explaining why.",
  ...
  {
    name: "triage",
    description: "Label an item and size it. labels: bug (something broken), copy (text change), design (layout/visual), content (client must write or supply content), photo (client must supply an image), decision (client must choose). effort: quick (under 30 minutes), medium (a small build or content pass), large (a real build). Pass only the fields you want to change.",
    inputSchema: { type: "object", required: ["id"], properties: { id: { type: "string" }, labels: { type: "array", items: { type: "string", enum: [...LABELS] } }, effort: { type: "string", enum: [...EFFORTS, ""] , description: "Empty string clears it." } } },
  },
...
async function callTool(project: Project, name: string, args: Record<string, any>) {
  switch (name) {
    case "list_feedback": return text(await listFeedback(project, { status: args.status ?? "open", page: args.page, label: args.label }));
...
Deno.serve(async (req) => {
  ...
  for (const m of messages) {
    ...
    try {
      switch (m.method) {
        case "initialize":
          out.push(rpc(m.id, { protocolVersion: PROTOCOL, capabilities: { tools: { listChanged: false } }, serverInfo: { name: "pinpoint", version: VERSION }, instructions: `Feedback for the website project "${project.name}". First pass: triage every open item that has no labels — set labels and effort, set_status waiting (with a reply saying what you need) on anything the client must decide or supply, and assign photo/content requests to the client contact if one is known. Then work through what you own, quick items first; when a change is made, set_status resolved with a short reply. Never resolve what you did not complete; if blocked, reply with why and leave it open. Before finishing, list_feedback again to confirm nothing you handled is still open.` }));
          break;
